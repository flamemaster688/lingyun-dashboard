# -*- coding: utf-8 -*-
"""为内联返回的 sheet5 / sheet8 生成紧凑 kdocs JSON（仅保留 rangeData 必要字段），
供 kdocs_to_csv.py 还原网格。数值与金山文档当前内容一致。"""
import json, os
...
def build_json(grid):
    ...
    return {
        "code": 0, "message": "成功",
        "data": {"detail": {"rangeData": cells}},
        "result": "ok",
    }


# sheet5 总览 1-7月调用量/节约人年/活跃智能体（含表头与月均值行）
sheet5 = [
    ["月份", "调用量", "节约人年", "活跃智能体总数"],
...
# sheet8 多能力等效人年（列：产品,灵运,RPA,教练,质检,立单,语音,4项总计,月累计）
sheet8 = [
...
for name, grid in (("_raw_sheet5.json", sheet5), ("_raw_sheet8.json", sheet8)):
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump(build_json(grid), f, ensure_ascii=False)
    print("OK ->", path, "(cells=%d)" % sum(1 for row in grid for v in row if v not in (None, "")))
