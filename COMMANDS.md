...
```
static/
├─ core/core.js          # 共享内核（全局脚本）：状态 D/FILTER/V、工具函数、离线 Excel取数管线、tab 注册表
├─ pages/
...
4. 赵莹统一把各人提供的离线 Excel用 Excel 取数、写入 `data.js`（页面只读，不碰取数逻辑）。
...
- **前 6 个页面需连数据源**（数据总览/智能体/平台分析/省份分析/质效分析/告警中心）：各负责人提供自己的离线 Excel，由赵莹统一经 Excel 取数写入 `data.js`。页面**只读 `data.js`**，浏览器不能直连离线 Excel（CORS+鉴权）。
